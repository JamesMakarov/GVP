...
public class RoupaInferiorEnums implements Initializable {

    @FXML
    private ListView<String> enumRoupaInferior;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        List<String> listaEnumStringRoupaInferior = StringEnum.getInstance().listaEnumStringRoupaInferior();
        for (String i : listaEnumStringRoupaInferior) {
            enumRoupaInferior.getItems().add(i);
        }

        enumRoupaInferior.setOnMouseClicked(event1 -> {
            if (event1.getClickCount() == 1) {
                ...
                String selecao = enumRoupaInferior.getSelectionModel().getSelectedItem();
                if (selecao != null) {
                    confirmar.setOnAction(event2 -> {
                        ...
                        RoupaInferior acessorio = organizadorDeItens.criarItem(StringEnum.getInstance().stringParaEnumRoupaInferior(selecao),name,color,size,type,status );
